...
public class VRMLiveMotionStreamerUsingCharacterController : MonoBehaviourPunCallbacks
{
    public GameObject Avatar;
    public int SerializationRate = 30;
    public GameObject HumanPoseSynchronizerPrefab;
    public GameObject AvatarPositionSynchronizerPrefab;
    ...
    public override void OnJoinedRoom()
    {
        ...
        if (AvatarPositionSynchronizerPrefab != null && Avatar != null)
        {
            GameObject positionSynchronizer = PhotonNetwork.Instantiate(this.AvatarPositionSynchronizerPrefab.name, SynchronizerPosition, Quaternion.identity, 0);
            positionSynchronizer.GetComponent<AvatarPositionSynchronizer>().AvatarPositionTransform = Avatar.transform;
            RaiseEventToSetAvatarPositionSynchronizer(positionSynchronizer);
        }
    }
    ...
    void RaiseEventToSetAvatarPositionSynchronizer(GameObject avatarPositionSynchronizer)
    {
        PhotonView photonView = avatarPositionSynchronizer.GetComponent<PhotonView>();
        if (photonView != null)
        {
            int sendData = photonView.ViewID;
            PhotonNetwork.RaiseEvent((byte)VRMLiveMotionEventCode.SetAvatarPositionSynchronizer, sendData, raiseEventOptions, sendOptions);
            Debug.LogFormat("PhotonNetwork.RaiseEvent: {0}", (byte)VRMLiveMotionEventCode.SetAvatarPositionSynchronizer);
        }
    }
}